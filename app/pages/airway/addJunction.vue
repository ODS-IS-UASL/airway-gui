...
import addJunctionSetting from "../components/airway/addJunctionSetting.vue"
import junctionNameSetting from "../components/airway/addJunctionNameSetting.vue"
import confirmation from "../components/airway/addJunctionConfirmation.vue"
import { ref } from "vue"
import { useRouter } from 'vue-router';
import LoadingDialog from '~/components/dialogs/LoadingSpiner.vue'

const step = ref(0)
const stepTitle = ref('航路点追加')
const nextButton = ref('航路点設定')
...
const handleBasicInfomationUpdate = (basicInfomation) => {
  ...
  corridorData.value = basicInfomation.corridorList
  baseSectionNames.value = basicInfomation.baseSectionNames
  modelNames.value = basicInfomation.modelNames
...
const showModal = async () => {
  ...
  const airwayRes = await $fetch(`/api/airway/uasl/${uaslId}`, { 
    method: 'PATCH',
  ...
  if (airwayRes.status === 200) {
    ...
    console.error(`error: patch airway info {status: ${airwayRes.status}}.`);
    isDialogVisible.value = true;
    console.error(`error: patch airway info {status: ${airwayRes.status}}.`);
    if (airwayRes.status == 422) {
      ...
      conflictAirwaySection.value = uaslSectionNames[airwayRes.data.details.uaslSectionIndex];
    } else {
  ...
  <main id="main" class="b-pageMain">
    <div class="b-pageContentHasSubMenu">
      ...
      <div class="b-pageContentHasNavigation">
        ...
        <v-stepper v-model="step">
          ...
          <v-stepper-window>
            ...
            >
              <!-- 詳細情報 -->
              <addJunctionSetting :stepNo="1" :key="junctionSettingKey" @update:basicInfomation="handleBasicInfomationUpdate" @update:junctionSettingCreateAirwayPoint="handlerIsJunctionSetting"/>
            </v-stepper-window-item>

            <v-stepper-window-item
            value="2"
            >
              <junctionNameSetting :stepNo="2" :key="junctionNameSettingKey" :rangeData="rangeData" @update:changedCorridorData="handleChangedCorridorDataUpdate"/>
            </v-stepper-window-item>
      ...
      <dialog class="c-dialog" v-if="isDialogVisible">
        ...
        <p v-if="createStatus === STATUS.CONFLICT">航路画定申請に失敗しました。<br>航路区画 {{ conflictAirwaySection }} で他航路と干渉しています</p>
        <ul class="e-buttonGroup">
