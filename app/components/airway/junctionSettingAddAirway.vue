...
import { ref } from "vue";
import CreateAirway from "~/components/map/createSharedAirway.vue";
...
export default {
  ...
  data() {
    return {
      ...
      sourceUaslId: this.rangeData.sourceUaslId,
      targetUaslId: this.rangeData.targetUaslId,
  ...
  methods: {
    ...
    handlerUpdateValue(newValue) {
      ...
      if (Object.keys(newValue).length  !== 0) {
        ...
        this.corridorList.sourceMarkerCoords = newValue.sourceMarkerCoords
        this.corridorList.targetMarkerCoords = newValue.targetMarkerCoords

        this.corridorList.airwaySections = [];

        if ('corridor_points' in newValue) {
          for (let i=0; i < newValue.corridor_points.length; i++) {
            ...
            let airway_junction_id = `airway_junction_id_${pointIndex}_${new Date().toISOString()}`
            if (i === 0) {
              airway_junction_id = newValue.sourcePointId
            } else if (i === newValue.corridor_points.length - 1) {
              airway_junction_id = newValue.targetPointId
            }
            ...
            // 新規航路改修用 start
            if (newValue.corridor_points[i].hasOwnProperty("LDN_externalGuarantee")) {
              airwayJunction.externalGuarantee = newValue.corridor_points[i].LDN_externalGuarantee;
            }
            if (newValue.corridor_points[i].hasOwnProperty("LDN_externalSystemInfo")) {
              airwayJunction.externalSystemInfo = newValue.corridor_points[i].LDN_externalSystemInfo;
            }
            // 新規航路改修用 end
