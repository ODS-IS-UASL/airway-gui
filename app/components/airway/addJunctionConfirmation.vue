...
export default {
  ...
  async setup(props, { emit }) {
    ...
    const modelNames = props.rangeData.modelNames;
    ...
    const item = fallToleranceRange.value["fallToleranceRanges"].find(item => item["fallToleranceRangeId"] === fallToleranceRangeId.value)
    const fallToleranceRangeName = item?.name;
    ...
    // despersion_nodes がどの新規航路点とマッチするか
    const despersionIdx = parsedcorridorData.airwayJunctions
        .map((item, index) => item.new ? index : -1)
        .filter(index => index !== -1);
    if (despersionIdx.length === parsedcorridorData.despersion_nodes.length) {
      for(let i=0; i<parsedcorridorData.despersion_nodes.length; i++) {
          parsedcorridorData.despersion_nodes[i].idx = despersionIdx[i]
      }
    }
    ...
    for (let i = 0; i < parsedcorridorData.airwayJunctions.length; i++) {
      ...
      let airwaySection = {};
      let airwayPart = {};
      if (parsedcorridorData.airwayJunctions[i].new) {
        // 新規航路点
        const despersion_nodes = parsedcorridorData.despersion_nodes.find(item => item.idx === i);
        despersionNode = {
          name: despersion_nodes.name,
          geometry: {
            coordinates: despersion_nodes.geometry.coordinates,
            type: 'LineString'
          },
          feasibleVolSeamId: despersion_nodes.feasibleVolSeamId
        }
        airwayJunction = [{
        ...
        // 既存航路点
        const externalPointId = parsedcorridorData.airwayJunctions[i].airwayJunctionId;
        if (i === 0) { // 始点はセクションの追加をしない
          airwayPart = {
    ...
    const postJsonData = convertAirwayToUaslAddJunction(postJsonDataTmp);
